...
function TierBanner({ tier = "Basic" }) {
  const style =
    tier === "VIP"
  ...
  return (
    <div className={`rounded-xl p-4 mb-6 ${style}`}>
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold">Your Plan: {tier}</h2>
          <p className={tier === "VIP" ? "text-black/80" : "text-white/80"}>{desc}</p>
        </div>
        <div className="flex gap-2">
          <a
            href={NAS_LINKS.monthly[tier]}
            target="_blank"
            rel="noreferrer"
...
export default function Dashboard() {
  ...
  const fetchAll = async () => {
    ...
    const { data: prof } = await supabase
      ...
      .single();
    setProfile(prof || { membership_tier: "Basic", email: userData.user.email });

    const { data: sigs } = await supabase
      .from("signals")
      .select("*")
      .gte("date", new Date(Date.now() - 8 * 24 * 60 * 60 * 1000).toISOString()) // last 8 days for history tab
  ...
  const limitedToday = useMemo(() => {
    ...
    if (tier === "VIP") return todays;
    if (tier === "Pro") return todays.slice(0, Math.min(8, todays.length));
  ...
  const historyRows = useMemo(() => {
    ...
    const today = new Date().toISOString().slice(0, 10);
    return signals.filter((s) => (s.date || "").slice(0, 10) !== today);
  }, [signals]);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-4">
          <button
            ...
            className={`px-4 py-2 rounded-lg border ${
              tab === "today" ? "border-yellow-400 text-yellow-400" : "border-gray-700 text-gray-300"
            }`}
          >
            Today
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-lg border ${
              tab === "history" ? "border-yellow-400 text-yellow-400" : "border-gray-700 text-gray-300"
            }`}
          >
            History (7d)
          </button>
          ...
          <button
            onClick={fetchAll}
            className="ml-auto px-3 py-2 rounded-lg bg-gray-800 hover:bg-gray-700 border border-gray-700"
          >
            Refresh
          </button>
        </div>
        ...
        <div className="overflow-x-auto border border-gray-800 rounded-lg mb-10">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr><td className="p-5" colSpan={7}>Loading…</td></tr>
              ) : (tab === "today" ? (limitedToday || []) : historyRows).length === 0 ? (
                <tr>
                  <td colSpan={7} className="p-5 text-center text-gray-400">
                    {tab === "today" ? "No signals yet today." : "No history available."}
                  </td>
                </tr>
              ) : (
                (tab === "today" ? limitedToday : historyRows).map((s) => (
